package com.iqmsoft.es.service;
...
import com.iqmsoft.es.repository.TripRepository;
import com.iqmsoft.es.repository.search.TripSearchRepository;
...
public class TripService {
	...
	public List<Trip> findByGeoLocationSort(Double latitude, Double longitude, Integer distance, DistanceUnit unit,
			SortOrder order) {
		GeoDistanceFilterBuilder filter = FilterBuilders.geoDistanceFilter("location").distance(distance, unit)
				.lat(latitude).lon(longitude);

		SearchQuery searchQuery = new NativeSearchQueryBuilder().withFilter(filter)
				.withSort(SortBuilders.geoDistanceSort("location").point(latitude, longitude).order(order)).build();

		return StreamSupport.stream(tripSearchRepository.search(searchQuery).spliterator(), false)
	...
	public List<Trip> findByGeoLocation2(Double latitude, Double longitude, Integer distance, DistanceUnit unit) {
		...
		return StreamSupport
				.stream(tripSearchRepository.search(new NativeSearchQuery(matchAllQuery(), boolFilter)).spliterator(),
						false)
				.collect(Collectors.toList());
	}
